...
from progenetix_publication.publication import Publication
            ...
            for fname in files:
                ...
                try:
                    if fname.endswith('.yaml'):
                        nyaml += 1
                        o: Publication = yaml_loader.load(full_fname, Publication)
                        pyaml += 1
                    elif fname.endswith('.json'):
                        njson += 1
                        o: Publication = json_loader.load(full_fname, Publication)
                        pjson += 1
                    elif fname.endswith('.ttl'):
                        nttl += 1
                        o: Publication = rdf_loader.load(full_fname, Publication)
                        pttl += 1
